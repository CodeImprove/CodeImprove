...
def main():
    ...
    if args.dissector:
            ...
            #im using test_data
            #train_dataset = TextDataset(tokenizer, args,args.eval_data_file)
            train_dataset = TextDataset(tokenizer, args, args.train_data_file)
            ...
            dev_dataset = TextDataset(tokenizer, args, args.eval_data_file)
            dev_sampler = SequentialSampler(dev_dataset) 
            dev_dataloader = DataLoader(dev_dataset, sampler=dev_sampler,batch_size=args.eval_batch_size,num_workers=4,pin_memory=True)

            dissector = PVScore(model=model, train_loader=train_dataloader, dev_loader=dev_dataloader, args=args)

            test_dataset =  TextDataset(tokenizer, args, args.test_data_file)
            test_sampler = SequentialSampler(test_dataset) 
            testdataloader = DataLoader(test_dataset,sampler=test_sampler, batch_size=args.eval_batch_size,num_workers=4,pin_memory=True)
            scores = dissector._uncertainty_calculate(testdataloader)
            print(scores)
            print(len(scores))
            scores = scores[0]
            print(scores[0])
            print(scores)
            with open(os.path.join(args.output_dir,"hidden_dissector_uncertainty.txt"),'w') as f:
                for example, sc in zip(test_dataset.examples,scores):
